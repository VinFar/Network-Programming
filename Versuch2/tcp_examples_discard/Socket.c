...
void *Senddate(thr_struct *attr){
	...
	int fd[CLIENT_CNT];
	char buf[BUFFER_SIZE][CLIENT_CNT];
	ssize_t len[CLIENT_CNT];

	fd[fd_index-1] = attr->connfd;

	do{

		printf("index: %d",(fd_index-1));
		len[fd_index-1] = Recv(fd[fd_index-1],buf[fd_index-1],sizeof(buf),0);
		puts("recvd");
		printf("%s",buf);
		sleep(1);
		
	}while(len[fd_index-1] > 0);
